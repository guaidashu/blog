...
var MPlayer = (function () {
    function MPlayer(settings, callback) {
        ...
        formatData(0, function () {
            // 切换播放模式
            $this.changePlayMode($this.settings.playMode);
            // 循环输出列表标题
            var listHTML = '';
            $.each($this.list, function (i) {
                listHTML += '<li data-category-id="' + $this.list[i].category_id + '" class="mp-list-title-' + i + '">' + $this.list[i].listName + '</li>';
            });
        ...
        function bindEvents() {
            // 专辑图片加载失败将使用默认图片
            // $this.dom.cover.errorCallback(function () {
            ...
            $this.dom.listTitle.on('click', 'li', function () {
                // 获取索引
                var list = $(this).index();
                category_id = $this.dom.listTitle.find('.mp-list-title-' + list).attr("data-category-id")
                $this.formatData(category_id, function () {
            ...
            $this.dom.list.on('dblclick', eleName, function () {
            ...
            // 列表点击切换歌曲
            $this.dom.list.on('dblclick', $this.settings.listEleName, function () {
                $this.play(parseInt($this.audio.attr('data-displayList')), $(this).index());
            });
            // 禁止点击
         ...
         * 格式化数据
         */
        function formatData(category_id, func) {
            $this.list = [];
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        for (var j = 0; j < list[i].length; j++) {
                            if (list[i][j].basic) {
                                $this.list[i].category_id = list[i][j].category_id
    ...
    $.extend(MPlayer.prototype, {
        ...
        play: function (list, song) {
            ...
            $this.dom.play.html('<img src="/static/images/assets/pause_btn.svg" style="position:relative; top: 8px; cursor:pointer;">');
        },
        /**
         * 暂停，如果已暂停就什么也不做
         */
        pause: function () {
            ...
            $this.dom.play.html('<img src="/static/images/assets/play_btn.svg" style="position:relative; top: 8px; cursor:pointer;">');
        },
        /**
         * 获取当前歌曲的总时长
         * @returns {number}
        ...
        getInfo: function (list, song) {
            ...
            if ($this.tmpSong) {
                if ($this.audio.attr("data-currentSong") != song || $this.audio.attr("data-currentList") != list) {
        ...
        mute: function () {
            ...
            $this._trigger('mute');
            $this.dom.vol.html('<img src="/static/images/assets/mute_btn.svg"/>')
        },
        /**
         * 取消静音，如果没有静音就什么都不做
         */
        cancelMute: function () {
            ...
            $this._trigger('mute');
            $this.dom.vol.html('<img src="/static/images/assets/voice_btn.svg"/>')
        },
        /**
         * 切换静音状态
         */
